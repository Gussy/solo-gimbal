        ...
        LoadAxisParmsStateInfo* load_ap_state_info)
{
    switch (md_parms->motor_drive_state) {
        ...
        case STATE_WAIT_FOR_AXIS_CALIBRATION_STATUS:
            ...
            if ((cb_parms->calibration_status[AZ] != GIMBAL_AXIS_CALIBRATION_REQUIRED_UNKNOWN) &&
                ...
                if ((cb_parms->calibration_status[AZ] == GIMBAL_AXIS_CALIBRATION_REQUIRED_TRUE) ||
                    ...
                    // If no axes require calibration, immediately send the perform calibration command.
                    // The calibration routine for each axis will recognize that it doesn't need to be calibrated,
                    // and the startup process will continue normally.  Also send a mavlink message indicating that none
                    // of the axes need calibration
                    send_mavlink_axis_calibration_status(GIMBAL_AXIS_CALIBRATION_REQUIRED_FALSE, GIMBAL_AXIS_CALIBRATION_REQUIRED_FALSE, GIMBAL_AXIS_CALIBRATION_REQUIRED_FALSE);
        ...
        case STATE_WAIT_FOR_AXIS_CALIBRATION_COMMAND:
            ...
            if (GetBoardHWID() == AZ) {
                static unsigned int message_decimation_count = 0;
                if (message_decimation_count++ >= (ISR_FREQUENCY * 1000)) {
