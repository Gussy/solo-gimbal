						       ...
						       uint32_t time_boot_ms, uint64_t time_utc, uint8_t estimator_type, float x, float y, float z, float vx, float vy, float vz, float ax, float ay, float az, const float *covariance)
{
	...
	_mav_put_float_array(buf, 48, covariance, 45);
		...
		mav_put_float_c2000(&(msg->payload64[0]), 36, ax);
		mav_put_float_c2000(&(msg->payload64[0]), 40, ay);
		mav_put_float_c2000(&(msg->payload64[0]), 44, az);
		mav_put_uint8_t_c2000(&(msg->payload64[0]), 228, estimator_type);
	
		mav_put_float_array_c2000(&(msg->payload64[0]), covariance, 48, 45);
...
#else
	return mav_get_uint8_t_c2000(&(msg->payload64[0]),  228);
#endif
}
...
#else
	return mav_get_float_array_c2000(&(msg->payload64[0]), covariance, 45,  48);
#endif
}
