...
typedef union {
    ...
    // Fault Messages
    struct {
		unsigned fault_code:6;  // Fault code, see enum CAND_fault_code
		unsigned s_id:3;        // Sender ID = bXXX see enum CAND_sender_id
    ...
    struct {
        ...
        unsigned d_id:3;      ///< Destination ID = bXXX see enum CAND_destination_id
    ...
    union {
    	struct {
    		unsigned param:5;   ///< Parameter ID or Flags
        ...
        struct {
			unsigned param_id:5;	///< Parameter ID (addr_mode = 1)
            unsigned addr_mode:1;   		///< Address Mode = b0 (Immediate Address in following 5 bits) or b1 (Parameter ID in following 5 bits))
            unsigned d_id:3;  	///< Destination ID = bXXX see enum CAND_destination_id
            unsigned m_id:2;      	///< Message ID = b10
            unsigned :16;
        ...
        struct {
        	unsigned param_reg:5;	///< Parameter Register (addr_mode = 0)
            unsigned addr_mode:1;   		///< Address Mode = b0 (Immediate Address in following 5 bits) or b1 (Parameter ID in following 5 bits))
            unsigned d_id:3;      ///< Destination ID = bXXX see enum CAND_destination_id
            unsigned m_id:2;        	///< Message ID = b10
            unsigned :16;
    ...
    struct {
        unsigned dbg_code:3;
        unsigned s_id:3;       ///< Sender ID = bXXX see enum CAND_sender_id
        unsigned d_id:3; 	///< Destination ID = b111
        unsigned m_id:2;      ///< Message ID = b10
        unsigned :16;
        unsigned :5;
    } debug;
